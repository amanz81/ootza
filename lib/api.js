...
export const getAdvice = async (count = 5) => {
  ...
  const adviceQuery = query(adviceRef, orderByChild('createdAt'), limitToLast(count * 2)); // Fetch more to account for potential empty entries
  ...
  return advices.slice(0, count); // Return only the requested number of non-empty advices
};

export const getTopAdvice = async (count = 5) => {
  const adviceRef = ref(db, 'advices');
  const adviceQuery = query(adviceRef, orderByChild('likes'), limitToLast(count * 2)); // Fetch more to account for potential empty entries
  const snapshot = await get(adviceQuery);
  const advices = [];
  snapshot.forEach((childSnapshot) => {
    const advice = { id: childSnapshot.key, ...childSnapshot.val() };
  ...
  return advices.sort((a, b) => b.likes - a.likes).slice(0, count); // Sort and return only the requested number of non-empty advices
};
